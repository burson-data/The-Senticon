...
class NewsAnalyzerApp:
        ...
        with st.sidebar:
            ...
            if enable_scraping:
                with st.expander("🔧 **Opsi Scraping**"):
                    use_manual_title = st.checkbox(
                        "Gunakan Judul dari Input Manual", 
                        value=True, 
                        help="Jika dicentang, judul yang diinput manual (URL[tab]Judul) akan diprioritaskan."
                    )
                ...
                use_manual_title = False
        
        return {
            'use_manual_title': use_manual_title,
    ...
    async def process_single_url_async(self, url_data: Dict, config: Dict) -> Dict:
        ...
        try:
            result = {'URL': url}
            # Prioritize manual title if provided and enabled
    ...
    async def process_urls_manual_async(self, url_data_list: List[Dict], config: Dict) -> List[Dict]:
        ...
        tasks = [self.process_single_url_async(url_data, config) for url_data in url_data_list]
    ...
    def display_results(self, results, config: Dict, is_excel_data: bool = False, column_mapping: Optional[Dict] = None):
    ...
    def run(self):
        ...
        url_data_list, uploaded_file, df, column_mapping = [], None, None, {}

        with tab1:
            url_input = st.text_area(
                "Masukkan URL (satu per baris):", height=250,
                placeholder="https://example.com/news1\nhttps://example.com/news2\nhttps://example.com/news3[tab]Ini Judul Manualnya"
            )
            if url_input:
                ...
                for line in lines:
                    if '\t' in line:
                        ...
                        url_data_list.append({'url': line.strip()})
        ...
        active_input_method = "URL Manual" if url_data_list else "Upload File Excel" if uploaded_file else None

        warnings = self.validate_configuration(config, [d['url'] for d in url_data_list], uploaded_file)
        if warnings:
        ...
        if st.button("🚀 Mulai Analisis", disabled=not active_input_method or bool(warnings), width='stretch', type="primary"):
            ...
            with st.spinner("Memproses data... Ini mungkin memakan waktu beberapa saat tergantung jumlah data."):
                ...
                if active_input_method == "URL Manual":
                    ...
                    self.display_results(results_df, config, is_excel_data=True, column_mapping=column_mapping)
